...
from config import (
    CAMERA_CONFIG, BALL_DETECTION, CIRCULAR_MASK, MOTOR_CONFIG,
    FRAME_CONFIG, IMU_CONFIG, CONTROL_CONFIG
)
...
class SoccerRobot:
    def __init__(self):
        ...
        self.picam2.configure(self.picam2.create_video_configuration(
            main={"size": (CAMERA_CONFIG["width"], CAMERA_CONFIG["height"]), "format": CAMERA_CONFIG["format"]}
        ))
        self.picam2.start()
        ...
        # Frame configuration from config
        ...
        try:
            while True:
                ...
                if ball_found:
                    ...
                    # Calculate turn adjustment for display (same as in motor commands)
                    turn_adjustment = error_x_norm * self.max_speed * self.turn_sensitivity
                ...
                time.sleep(CONTROL_CONFIG["update_frequency"])  # Control loop frequency from config
